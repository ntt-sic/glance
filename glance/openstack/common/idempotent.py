...
from glance.openstack.common import memorycache
...
def get_request(*args, **kwargs):
    ...
    for target in targets:
        if target in kwargs:
            ...
            body = kwargs[target].body if hasattr(kwargs[target], 'body') else kwargs['body']
            if hasattr(kwargs[target], 'context'):
                tenant_id = kwargs[target].context.project_id
            else:
                for c in ('nova.context'):
...
def idempotent(f):
    ...
    def inner(*args, **kwargs):
        LOG.debug('####### Start POST execution #####')
        client_token = None
        value_list = []
        ...
        for key in headers.iterkeys():
            ...
            if key == "x-client-token":
                ...
                LOG.debug('####### X-CLIENT-TOKEN %s #####' % client_token)
        ...
        if client_token:
            ...
            LOG.debug('####### SUBSTANCE %s #####' % action)

            ### check client_token is used or not
            if stored and stored[1] == url:
                image_meta = stored[2]
                del(image_meta['status'])
                del(image_meta['size'])

                if image_meta == body and stored[3] == tenant_id:
